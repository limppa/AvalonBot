...
# SCREEN
screen = screen_utils.initialize_screen()
resx = 1920  # screen width
resy = 1080  # screen height

# LOADING IMAGES
img = image_utils.load_images() # test this without assigning to variable
bg = img['castlewall']
...
def place_pointer(xpos):
    ...
    bg.blit(img['pointer'], (int(xpos - pointer_w/2), int(Y_QUEST_CIRCLES - 1.9 * QUEST_CIRCLE_R)))
    screen.blit(bg, (0, 0))
    pg.display.flip()
...
def display_two_fails_required():
    bg.blit(img['twofailsreq'], (X_QUEST_CIRCLES[3] - QUEST_CIRCLE_R - 13, Y_QUEST_CIRCLES - QUEST_CIRCLE_R - 27))
