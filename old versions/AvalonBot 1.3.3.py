...
def quest_stamp():
    global quest_count
    if event.key == pg.K_s:
        circle_color_overlay(X_QUEST_CIRCLE[quest_count], Y_QUEST_CIRCLE, QUEST_CIRCLE_R, blueMark)
        freeze_quest_timer()
        quest_success()
        quest_count += 1
    if event.key == pg.K_f:
        circle_color_overlay(X_QUEST_CIRCLE[quest_count], Y_QUEST_CIRCLE, QUEST_CIRCLE_R, redMark)
        freeze_quest_timer()
        quest_fail()
        quest_count += 1
...
def total_timer():
    ...
    text = font.render("{0:02}".format(total_mins), 1, darkestGray)  # zero-pad Minutes to 2 digits
    textpos = text.get_rect(center=(xpos - QUEST_CIRCLE_R//3, ypos + TIMER_CIRCLE_R // 2.35))
    bg.blit(text, textpos)

    text = font.render(":{0:02}".format(total_secs), 1, darkestGray)
    textpos = text.get_rect(center=(xpos + QUEST_CIRCLE_R//3, ypos + TIMER_CIRCLE_R // 2.35))
    bg.blit(text, textpos)
...
def start_quest_timer():
    ...
    # quest_mins
    text = font.render("{0:02}".format(quest_mins), 1, darkerGray)  # zero-pad Minutes to 2 digits
    textpos = text.get_rect(center=(X_TIMER_CIRCLE[quest_count] - x_offset_mins, Y_TIMER_CIRCLE))
    bg.blit(text, textpos)
    # quest_secs
    text = font.render(":{0:02}".format(quest_secs), 1, darkerGray)
...
def clear_board_bottom():
    ...
    pg.draw.rect(bg, darkGray, [rect_x, rect_y, rect_width, rect_height])
...
while game_running:
    ...
    if success_count == 3 and assassin_phase is True and fail_count < 3:
        assassination()
        for event in pg.event.get():
            if event.type == pg.KEYDOWN:
                esc_quits(event)
            ...
            if event.type == clock_tick:
                clocks_increase()
                total_timer()
    elif success_count == 3 and assassin_phase is False and main_menu is False:
        good_wins()
        for event in pg.event.get():
            if event.type == pg.KEYDOWN:
                esc_quits(event)
