...
def play_success_sound():
    pg.mixer.music.load('sounds/success.mp3')
    pg.mixer.music.play()


def play_fail_sound():
    pg.mixer.music.load('sounds/fail.mp3')
    pg.mixer.music.play()
...
def display_game_board():
    ...
    pg.draw.rect(bg, border_color, [board_x, board_y - border_height, board_width, border_height])
...
def display_welcome_screen():
    ...
    bg.blit(IMG_welcometoavalon, imgpos.topleft)
...
def place_pointer(xpos):
    pointerw = IMG_pointer.get_rect().width  # 43
    pointerh = IMG_pointer.get_rect().height  # 71

    bg.blit(IMG_pointer, (int(xpos - pointerw/2), int(Y_QUEST_CIRCLES - 1.9 * QUEST_CIRCLE_R)))
    screen.blit(bg, (0, 0))
    pg.display.flip()
...
def clear_pointers():
    ...
    for position in range(0, 5):
        pg.draw.rect(bg, darkGray, [int(X_QUEST_CIRCLES[position] - pointerw/2),
                                    int(Y_QUEST_CIRCLES - 1.9 * QUEST_CIRCLE_R), pointerw, pointerh])

    screen.blit(bg, (0, 0))
    pg.display.flip()


def move_pointer(xpos, nextquest):
    pointerw = IMG_pointer.get_rect().width  # 43
    pointerh = IMG_pointer.get_rect().height  # 71
    pg.draw.rect(bg, darkGray, [int(xpos - pointerw/2), int(Y_QUEST_CIRCLES - 1.9 * QUEST_CIRCLE_R), pointerw, pointerh])
    bg.blit(IMG_pointer, (int(nextquest - pointerw/2), int(Y_QUEST_CIRCLES - 1.9 * QUEST_CIRCLE_R)))
    screen.blit(bg, (0, 0))
    pg.display.flip()
...
def total_timer():
    ...
    textrect.center = (xpos, ypos - TIMER_CIRCLE_R // 2)
    bg.blit(text, textrect)

    font = pg.font.SysFont("Calibri", 48)
    # total_minutes
    minutestext = font.render("{0:02}".format(total_minutes), 1, darkestGray)  # zero-pad Minutes to 2 digits
    minutestextrect = minutestext.get_rect()
    minutestextrect.center = (xpos - QUEST_CIRCLE_R//3, ypos + TIMER_CIRCLE_R // 2.35)
    bg.blit(minutestext, minutestextrect)
    # quest_seconds
    secondstext = font.render(":{0:02}".format(total_seconds), 1, darkestGray)
    secondstextrect = secondstext.get_rect()
    secondstextrect.center = (xpos + QUEST_CIRCLE_R//3, ypos + TIMER_CIRCLE_R // 2.35)
...
def quest_timer(xpos):
    ...
    draw_circle(bg, xpos + TIMER_CIRCLE_OFFSET, Y_TIMER_CIRCLES, int(TIMER_CIRCLE_R - 0.07 * TIMER_CIRCLE_R), redMark) # inner ring
    # quest_minutes
    minutestext = font.render("{0:02}".format(quest_minutes), 1, darkestGray)  # zero-pad Minutes to 2 digits
    minutestextrect = minutestext.get_rect()
    minutestextrect.center = (xpos + TIMER_CIRCLE_OFFSET - 18, Y_TIMER_CIRCLES)
    bg.blit(minutestext, minutestextrect)
    # quest_seconds
    secondstext = font.render(":{0:02}".format(quest_seconds), 1, darkestGray)
    secondstextrect = secondstext.get_rect()
    secondstextrect.center = (xpos + TIMER_CIRCLE_OFFSET + 15, Y_TIMER_CIRCLES)
    bg.blit(secondstext, secondstextrect)
    screen.blit(bg, (0, 0))
    pg.display.flip()
    total_timer()
...
def freeze_quest_timer(xpos):
    ...
    # quest_minutes
    minutestext = font.render("{0:02}".format(quest_minutes), 1, darkerGray)  # zero-pad Minutes to 2 digits
    minutestextrect = minutestext.get_rect()
    minutestextrect.center = (xpos + TIMER_CIRCLE_OFFSET - 18, Y_TIMER_CIRCLES)
    bg.blit(minutestext, minutestextrect)
    # quest_seconds
    secondstext = font.render(":{0:02}".format(quest_seconds), 1, darkerGray)
    secondstextrect = secondstext.get_rect()
    secondstextrect.center = (xpos + TIMER_CIRCLE_OFFSET + 15, Y_TIMER_CIRCLES)
    bg.blit(secondstext, secondstextrect)
    screen.blit(bg, (0, 0))
...
def player_box():
    displayh = int(1.4 * QUEST_CIRCLE_R)  # if changing value, check assassination function
    displayw = int((GAME_BOARD_HEIGHT / GOLDEN_RATIO)/2)    # if changing value, check assassination function
    displayw2 = int(displayw - 0.5 * TIMER_CIRCLE_R)
    displayh2 = int(displayh - 0.5 * TIMER_CIRCLE_R)

    pg.gfxdraw.box(bg, [int(resx - ((resx - GAME_BOARD_WIDTH) / 2) - displayw),
                        int(resy - ((resy - GAME_BOARD_HEIGHT) / 2) - 2.4 * QUEST_CIRCLE_R), displayw, displayh], lightGray)
    pg.gfxdraw.box(bg, [int(resx - ((resx - GAME_BOARD_WIDTH) / 2) - displayw + (displayh - displayh2)/2),
...
def display_msg(msg):
    ...
    leftscroll_xpos = resx / 2 - scrollw / 2 - displayw / 2 + 5
    rightscroll_xpos = resx / 2 - scrollw / 2 + displayw / 2 - 5
    scroll_ypos = 272
    ...
    if voting_phase and success_count < 3 and fail_count < 3:
        ...
        if quest_count in [0, 1, 2]:
            ...
            for event in pg.event.get():
                ...
                if event.type == clock_tick:
                    clocks_increase()
                if quest_count < len(X_QUEST_CIRCLES):  # Check if quest_count is within the valid range
                    quest_timer(X_QUEST_CIRCLES[quest_count])
